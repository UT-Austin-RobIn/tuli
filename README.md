...
2. Check if NAS (synology) is mounted:
  un: `mountpoint -q ~/synology-tuli && echo "Mounted" || echo "Not mounted"`
  f output is "Not mounted":
...
1. Activate the virtualenv, `source ~/envs/infants/bin/activate` and run the experiment script:
...
1. Press ENTER to stop recording that trial.
2. Say `[y/n]` to keep trial or delete.
3. Press `ctrl+c` to interrupt and kill the program.
